---
title: "Presence of voiced uvular stop"
---
...
bib_uv <- ReadBib(file = "bibliography.bib")
...
uv <- read_tsv("data/voiced_uvular.tsv") # feature dataset
...
uv_vill <- merge(vill_meta, uv, by = "lang") # merge villages, coordinates, and language metadata with feature information

uv_vill$datapoint <- "extrapolated datapoint"
...
map.feature(lang.gltc(uv_vill$glottocode),
            latitude = uv_vill$lat,
            longitude = uv_vill$lon,
            features = uv_vill$lang, # color feature = language
            color = uv_vill$lang_color_dagatlas,
            stroke.features = uv_vill$value, # stroke.feature = your feature value
            stroke.color = c("black", "white"), 
            label = uv_vill$lang,
            zoom.control = T,
            popup = paste("<b>Village:</b>", uv_vill$village, "<br>", 
                          "<b>Source:</b>", uv_vill$source, uv_vill$page, "<br>",
                          "<b>Datapoint:</b>", uv_vill$datapoint),
...
core_data <- left_join(core_meta, uv, by = "lang")
...
for (i in seq_along(dtable$source)) 
  ...
  cits = strsplit(dtable$source[[i]], "; ")
    for (j in seq_along(cits))
      { 
        cits[[j]] <- RefManageR::Cite(bib = bib_uv, cits[[j]], .opt = list(max.names = 2))
      }
  dtable$source[[i]] = paste(cits, sep = "; ")
    } 
  else 
    {
      dtable$source[[i]] <- RefManageR::Cite(bib = bib_uv, dtable$source[[i]], .opt = list(max.names = 2))
    }
}
...
PrintBibliography(bib_uv, .opts = list(sorting = "nyt"))
```