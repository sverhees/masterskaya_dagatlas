...
bib_nas <- ReadBib(file = "bibliography.bib")
...
nas <- read_tsv("data/nasalization.tsv") # feature dataset
...
nas_vill <- merge(vill_meta, nas, by = "lang") # merge villages, coordinates, and language metadata with feature information

nas_vill$datapoint <- "extrapolated datapoint"
...
map.feature(lang.gltc(nas_vill$glottocode),
            latitude = nas_vill$lat,
            longitude = nas_vill$lon,
            features = nas_vill$lang, # color feature = language
            color = nas_vill$lang_color_dagatlas,
            stroke.features = nas_vill$value, # stroke.feature = your feature value
            stroke.color = c("black", "white"), 
            label = nas_vill$lang,
            zoom.control = T,
            popup = paste("<b>Village:</b>", nas_vill$village, "<br>", 
                          "<b>Source:</b>", nas_vill$source, nas_vill$page, "<br>",
                          "<b>Datapoint:</b>", nas_vill$datapoint),
...
core_data <- left_join(core_meta, nas, by = "lang")
...
for (i in seq_along(dtable$source)) 
  ...
  cits = strsplit(dtable$source[[i]], "; ")
    for (j in seq_along(cits))
      { 
        cits[[j]] <- RefManageR::Cite(bib = bib_nas, cits[[j]], .opt = list(max.names = 2))
      }
  dtable$source[[i]] = paste(cits, sep = "; ")
    } 
  else 
    {
      dtable$source[[i]] <- RefManageR::Cite(bib = bib_nas, dtable$source[[i]], .opt = list(max.names = 2))
    }
}
...
PrintBibliography(bib_nas, .opts = list(sorting = "nyt"))
```